...
func (s *WSSource) Subscribe(ctx context.Context, eventKey string) error {
	if eventKey == "" {
		return exception.InvalidArgsError.With("wssource: empty eventKey")
	}
	return s.subscribeKeys(ctx, []string{eventKey})
}
...
// upstream call rather than firing N individual frames.
func (s *WSSource) SubscribeBatch(ctx context.Context, eventKeys []string) error {
	if len(eventKeys) == 0 {
		return nil
	}
	return s.subscribeKeys(ctx, eventKeys)
}
...
func (s *WSSource) subscribeKeys(ctx context.Context, eventKeys []string) error {
	sess := s.connState.Load()
	if sess == nil {
		return errors.New("wssource: not connected (yet); subscribe will be retried on reconnect Replay")
	}

	wire, msgID, err := wsspb.EncodeSubscribeReq(&s.seq, eventKeys)
	if err != nil {
