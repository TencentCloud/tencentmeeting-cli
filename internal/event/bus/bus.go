...
// indefinitely.  30 s is a conservative default chosen for this project.
...
func (b *Bus) Run(ctx context.Context) error {
	...
	//    len(b.conns) to handle the stale-tick race.
...
func (b *Bus) startSources(ctx context.Context) {
	...
	if upstream != nil {
		...
		// will retry.
		b.hub.SetOnFirstSubscribe(func(eventKey string) {
			if err := upstream.Subscribe(ctx, eventKey); err != nil {
		...
		for _, s := range srcs {
			...
			rn.SetOnReconnected(func() {
				...
				if batch, ok := upstream.(interface {
					SubscribeBatch(ctx context.Context, eventKeys []string) error
				}); ok {
					if err := batch.SubscribeBatch(ctx, keys); err != nil {
						b.logger.Warnf(ctx, "bus: upstream replay (batch) failed: %v", err)
					}
					return
				}
				for _, k := range keys {
					if err := upstream.Subscribe(ctx, k); err != nil {
						b.logger.Warnf(ctx, "bus: upstream replay(%s) failed: %v", k, err)
					}
				}
			})
		}
