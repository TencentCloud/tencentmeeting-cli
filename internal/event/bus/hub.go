...
//
// The wemeet hub design is deliberately simple:
//
//   - No subscriptionID / cleanupInProgress dance: our protocol has no
//     PreShutdownCheck/Ack, so there is no cleanup-lock TOCTOU race to guard.
...
type Hub struct {
	...
	// skips the call.
	onFirstSubscribe func(eventKey string)
...
func (h *Hub) SetOnFirstSubscribe(fn func(eventKey string)) {
	h.mu.Lock()
	defer h.mu.Unlock()
...
func (h *Hub) Publish(raw *eventruntime.RawEvent) {
	...
	// sharing across subscribers is safe.  (We have no per-subscriber seq
	// field, so a single shared instance is sufficient.)
